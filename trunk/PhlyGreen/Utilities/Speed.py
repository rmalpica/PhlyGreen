...
def Mach2CAS(Mach,h, DISA=0.):
    ...
    return np.sqrt(5)*ISA.a0*np.sqrt((qc/ISA.p_sls + 1)**ISA.delta - 1)
...
def CAS2TAS(CAS,h,DISA=0.):
    C1 = ((CAS**2)/(5*ISA.a0**2) + 1)**(1/delta) - 1
    return np.sqrt(5) * soundspeed(h,DISA) * np.sqrt( ((p_sls/Pstd(h)) * C1 + 1 ) ** delta - 1)
