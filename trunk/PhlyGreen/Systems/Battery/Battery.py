...
#determine battery configuration
    ...
    def Power_2_Current(self, SOC, Power_out):
        Resistance = self.cell_resistance * self.series_stack_size
        U_oc = self.SOC_2_OC_Voltage(SOC)
        U_out = (U_oc + (U_oc**2 - 4 * Power_out * Resistance)**0.5)/2 #this comes from the solution of P_out = U_out * I_out
    ...
    def Power_2_Parallel_Cells(self, SOC, Power_out):
        required_current = self.Power_2_Current(SOC, Power_out)
        parallel_cells = np.ceil(required_current/self.cell_current)
