...
class Constraint:
    ...
    def EvaluateConstraints(self, WTOoS, DISA):
        ...
        if len(self.CruiseConstraints) > 0:
            self.PWCruise = (1.0/self.aircraft.powertrain.PowerLapse(self.CruiseConstraints['Altitude'],DISA)) * self.aircraft.performance.PoWTO(self.WTOoS, self.CruiseConstraints['Beta'], 0, 1., self.CruiseConstraints['Altitude'], DISA, self.CruiseConstraints['Speed'], self.CruiseConstraints['Speed Type'])
        else:
        ...
        if len(self.TakeOffConstraints) > 0:
            ...
            self.PWTakeOff = self.aircraft.performance.TakeOff(WTOoS,self.TakeOffConstraints['Beta'], self.TakeOffConstraints['Altitude'], self.TakeOffConstraints['kTO'], self.TakeOffConstraints['sTO'], DISA, self.TakeOffConstraints['Speed'], self.TakeOffConstraints['Speed Type'])
        else:
        ...
        if len(self.AEOClimbConstraints) > 0:
            ...
            self.PWAEOClimb = (1.0/self.aircraft.powertrain.PowerLapse(self.AEOClimbConstraints['Altitude'],DISA)) * self.aircraft.performance.PoWTO(self.WTOoS, self.AEOClimbConstraints['Beta'], self.AEOClimbConstraints['ROC'], 1., self.AEOClimbConstraints['Altitude'], DISA, self.AEOClimbConstraints['Speed'], self.AEOClimbConstraints['Speed Type'])
        else:
        ...
        if len(self.OEIClimbConstraints) > 0:
            ...
            self.PWOEIClimb = (1.0/self.aircraft.powertrain.PowerLapse(self.OEIClimbConstraints['Altitude'],DISA)) * self.aircraft.performance.OEIClimb(self.WTOoS, self.OEIClimbConstraints['Beta'], self.OEIClimbConstraints['Speed'] * self.OEIClimbConstraints['Climb Gradient'], 1., self.OEIClimbConstraints['Altitude'], DISA, self.OEIClimbConstraints['Speed'], self.OEIClimbConstraints['Speed Type'])
        else:
        ...
        if len(self.AccelerationConstraints) > 0:
            ...
            self.PWAcceleration = (1.0/self.aircraft.powertrain.PowerLapse(self.AccelerationConstraints['Altitude'],DISA)) * self.aircraft.performance.PoWTO(self.WTOoS,self.AccelerationConstraints['Beta'], PsAcceleration, 1., self.AccelerationConstraints['Altitude'], DISA, Mavg, 'Mach')
        else:
            self.PWAcceleration = np.zeros(len(self.WTOoS))

        # TURN
        if len(self.TurnConstraints) > 0:
            self.PWTurn = (1.0/self.aircraft.powertrain.PowerLapse(self.TurnConstraints['Altitude'],DISA)) * self.aircraft.performance.PoWTO(self.WTOoS, self.TurnConstraints['Beta'], 0, self.TurnConstraints['Load Factor'], self.TurnConstraints['Altitude'], DISA, self.TurnConstraints['Speed'], self.TurnConstraints['Speed Type'])
        else:
            self.PWTurn = np.zeros(len(self.WTOoS))

        # CEILING
        if len(self.CeilingConstraints) > 0:
            self.PWCeiling = (1.0/self.aircraft.powertrain.PowerLapse(self.CeilingConstraints['Altitude'],DISA)) * self.aircraft.performance.Ceiling(WTOoS, self.CeilingConstraints['Beta'], self.CeilingConstraints['HT'], 1., self.CeilingConstraints['Altitude'], DISA, self.CeilingConstraints['Speed'])
        else:
            self.PWCeiling = np.zeros(len(self.WTOoS))


        if len(self.LandingConstraints) > 0:
            self.PWLanding, self.WTOoSLanding = self.aircraft.performance.Landing(WTOoS, self.LandingConstraints['Altitude'], self.LandingConstraints['Speed'], self.LandingConstraints['Speed Type'], DISA)
        else:
