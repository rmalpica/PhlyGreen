...
if __name__ == '__main__':
	...
	e_bat = np.linspace(ebat_min, ebat_max, num = grid_n)
	...
	for i in range(len(samples)):
		eBat = np.append(eBat,samples[i][0])
		Phi = np.append(Phi,samples[i][1])
	...
	P1 = np.reshape(Phi, [grid_n,grid_n])
	...
	ax = plt.gca()
	ax.set_xlabel(r'$e_{bat}$ [Wh/kg]', fontsize=12)
	ax.set_ylabel(r'$\varphi_{crz}$ [-]', fontsize=12)
	plt.savefig('Source_VS_phi.png', dpi=600, format='png',bbox_inches="tight")
	#plt.show()
